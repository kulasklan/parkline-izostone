...
class FiltersManager {
    ...
    initializeSliders() {
        ...
        const visibleApartments = this.getVisibleApartmentsForCurrentView();
        console.log('🏠 Initializing sliders for', visibleApartments.length, 'visible apartments');
        ...
        // Extract floor data from visible apartments only
        console.log('🏢 Extracting floor data from visible apartments...');
        const validFloors = [];
        ...
        // Extract area data with proper numeric conversion
    ...
    updateExistingSliders() {
        // Recalculate ranges for current view
        const visibleApartments = this.getVisibleApartmentsForCurrentView();
        
        if (visibleApartments.length > 0) {
            // Update floor range
            const validFloors = visibleApartments
                .map(apt => parseInt(apt.floor))
            ...
            if (validFloors.length > 0) {
                ...
                this.allAvailableFloors = [minFloor, maxFloor];
                this.currentFilters.floors = [minFloor, maxFloor];
                this.updateSliderRange('floor', minFloor, maxFloor);
            }
            
            // Update area range with proper buffer
            const validAreas = visibleApartments
                .map(apt => parseFloat(apt.area))
                .filter(a => !isNaN(a) && a > 10);
            
            if (validAreas.length > 0) {
                const minArea = Math.min(...validAreas);
                const maxArea = Math.max(...validAreas);
                
                // Add buffer to ensure all apartments are included
                const minAreaWithBuffer = Math.max(Math.floor(minArea - 10), 10);
                const maxAreaWithBuffer = Math.ceil(maxArea + 10);
                
                this.allAvailableAreas = [minAreaWithBuffer, maxAreaWithBuffer];
                this.currentFilters.area = [minAreaWithBuffer, maxAreaWithBuffer];
                this.updateSliderRange('area', minAreaWithBuffer, maxAreaWithBuffer);
                
                console.log(`🔄 Updated area range for view: ${minAreaWithBuffer} - ${maxAreaWithBuffer} m²`);
            }
        }
        
        this.updateSliderDisplay('floor');
        this.updateSliderDisplay('area');
    }
    ...
    clearAllFilters() {
        ...
        if (this.clearAllState === 'active') {
            console.log('🧹 Clearing all filters - HIDING ALL APARTMENTS');
            ...
            console.log('🔄 Restoring filters - SHOWING ALL AVAILABLE BEDROOMS');
            
            // ALWAYS restore ALL available bedrooms
