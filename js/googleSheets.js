...
class GoogleSheetsManager {
    ...
    getFilteredApartments(filters) {
        ...
        const filtered = this.apartments.filter(apartment => {
            // BEDROOM FILTER: If no bedrooms selected, hide all apartments
            const matchesBedrooms = filters.bedrooms.length === 0 ? false : filters.bedrooms.includes(apartment.bedrooms);
            
            // FLOOR FILTER: Numeric comparison
            ...
            // AREA FILTER: Numeric comparison - FIXED
            ...
            if (this.debugMode && !matches) {
                console.log(`❌ ${apartment.id}: bedrooms=${matchesBedrooms}(${apartment.bedrooms}), floor=${matchesFloor}(${apartmentFloor}), area=${matchesArea}(${apartmentArea}), status=${matchesStatus}(${apartment.status})`);
            }
