...
${i18nManager.t('load-time')} ${Date.now() - this.loadingStartTime}ms
    ...
    // Handle escape key
    handleEscapeKey() {
        if (analyticsManager && analyticsManager.isVisible) {
            analyticsManager.hideDashboard();
        } else if (apartmentDetailsManager && apartmentDetailsManager.isVisible) {
            apartmentDetailsManager.hideDetails();
        }
    }
    ...
    handleOutsideClick(e) {
        ...
        if (analyticsManager && analyticsManager.isVisible && e.target.id === 'analyticsModal') {
            analyticsManager.hideDashboard();
        }
    }
